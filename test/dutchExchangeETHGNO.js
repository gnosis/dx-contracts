...
contract('DutchExchangeETHGNO', function(accounts) {
	...
	beforeEach(async function() {
		...
		// get seller set up  
    seller = accounts[1];
    sellToken = await TokenETH.at("0x65935656809c1bbc223c12548599c0227be39ceb"); 
    console.log('STA = ',sellToken.address)
		await sellToken.approve(seller, 100);
 		await sellToken.transferFrom(initialiser, seller, 100, {from: seller});

		// get buyer set up
		buyer = accounts[2];
		buyToken = await TokenGNO.at("0x2bfc1db41c85370ec987cb58abd67ec9ef7d5421");
		await buyToken.approve(buyer, 1000);
		await buyToken.transferFrom(initialiser, buyer, 1000, {from: buyer});

		TUL = await Token.at("0x2eeb2bd5b288f03ae8836fb696e712ea0c12e966");
    
		// create dx
		dx = await DutchExchangeETHGNO.at("0xa2f348d63e2de6a2446b5c907315c3ac8a549660");
    dxa = dx.address;
    console.log('DXA = ',dxa)
	})
    ...
    console.log('dx.sellToken ADDR = ', exchangeSellToken)
